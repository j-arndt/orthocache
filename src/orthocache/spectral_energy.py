...
def _run_fwht_on_blocks(keys: jax.Array, block_size: int = 512):
...
def compute_query_aware_bounds(
    ...
    ac_energy = jnp.sum(ac_components ** 2, axis=(1, 3))  # (num_blocks, num_heads)
    ...
    residual_bound = (
        q_norm[:, jnp.newaxis, :] * jnp.sqrt(ac_energy)[jnp.newaxis, :, :]
    ) / jnp.sqrt(head_dim)
