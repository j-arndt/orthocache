...
Across all 60 model layers, we observe zero truncation bound violations,
FWHT Parseval identity to $< 3 \times 10^{-7}$ relative error, and a
$\zeta$ separability ratio of $4.04 \times 10^{12}$ between blocks with
identical spatial variance---confirming the spectral transform is
non-redundant. The current Pallas kernel achieves full memory savings
(50\% KV-cache reduction) but does not yet translate to wall-clock
latency improvement due to the MXU's inability to skip masked tile
operations.
\end{abstract}
...
The $1.05\times$ speedup at 8K tokens (2.31\,ms vs.\ 2.44\,ms) is within
measurement noise ($\sigma \approx 0.06$--$0.12$\,ms). At all other
sequence lengths, sparse and dense attention are within 0.1\% of each other.
...
\noindent\textbf{Caveat.} The current prototype kernel achieves latency
parity with dense attention (speedup $\approx 1.00\times$ across all tested
sequence lengths). While this confirms zero overhead from the masking
approach, the economic value materializes only when the sparse kernel
...
\begin{enumerate}
    ...
    \item \textbf{Irregular AllToAll collectives}: In multi-host
          ...
          the ICI fabric---physically preventing evicted data from
          saturating bisection bandwidth.
\end{enumerate}
