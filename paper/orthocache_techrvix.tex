...
KV-cache, achieving 2--4$\times$ memory reduction.
OCTOPUS~\cite{octopus} extends this paradigm with octahedral parametrization
for joint triplet quantization of rotation-preconditioned coordinates.
KIVI~\cite{kivi} introduces
...
Adamas~\cite{adamas} applies Hadamard transforms with bucketization and 2-bit
compression for top-$k$ token selection via Manhattan-distance estimation.
While Adamas operates at per-token granularity during the attention kernel,
OrthoCache evicts at block granularity \emph{before} attention is computed,
using spectral band decomposition rather than compressed similarity search.
...
under optimal squared error quantization,''
arXiv preprint arXiv:2605.21226, 2025.

\bibitem{adamas}
D.~Zhu, C.~Li, Z.~Chen, B.~Yuan, and J.~Zhao,
``Adamas: Hadamard sparse attention for efficient long-context inference,''
arXiv preprint arXiv:2510.18413, 2024.
